...
- **Add unit tests for `shellTokenize`** — covers POSIX and Windows branches including path preservation, MSVC backslash-quote counting, empty quoted tokens, and edge cases. Tests run in CI (`build.yml`) and fail the workflow on regression.
